...
func _process(delta: float) -> void:
	...
	if KeyUtils.is_action_just_pressed(player.control_schema, KeyUtils.Action.SHOOT):
		...
		var player_state_data = PlayerStateData.new()
		player_state_data.shot_direction = shot_direction
		player_state_data.shot_power = shot_power
		transition_state(Player.State.SHOOTING, player_state_data)
